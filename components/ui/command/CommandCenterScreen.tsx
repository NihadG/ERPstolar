...
// prebacivanjem.
//
// Raspored se prilagođava sadržaju u tri nivoa:
//   1. dvije kolone (normalno)
//   2. fokus — prošireni nalog/proizvod uzme punu širinu, traka pada ispod
//   3. solo — jedna ploča preko svega, ostale postanu trake s brojem
// ════════════════════════════════════════════════════════════════════

import { useCallback, useEffect, useMemo, useState } from 'react';
import { createPortal } from 'react-dom';
import { ArrowLeft, Eye, EyeOff, LayoutDashboard, Printer, RefreshCw, X } from 'lucide-react';
...
import { addToBoard, boardProjects, removeFromBoard } from '@/lib/command/board';
import { buildScope, type BoardScope } from '@/lib/command/scope';
import { commandMaterialRows, planFromSelection } from '@/lib/command/materialOrder';
...
export default function CommandCenterScreen(props: CommandCenterScreenProps) {
    ...
    const [wizardFor, setWizardFor] = useState<WizardInitialProducts | null>(null);
    ...
    const boardRaw = useMemo(() => boardProjects(board, projects), [board, projects]);
    ...
    useEffect(() => {
        if (embedded) return;
        const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape' && !document.querySelector('.modal-overlay')) onClose(); };
    ...
    // ── Nalog iz odabranih proizvoda ─────────────────────────────────
    const createWorkOrderFor = (projectId: string) => {
        const project = onBoard.find(p => p.Project_ID === projectId);
        if (!project) return;
        const chosen = (project.products || []).filter(p => selectedProducts.has(p.Product_ID));
    ...
    // ── Narudžba iz odabranih materijala ─────────────────────────────
    const orderPlan = useMemo(() => planFromSelection(materials, selectedMaterials), [materials, selectedMaterials]);
    const expectedDelivery = shiftDate(today, 7);

    const createOrders = async (ids: string[]) => {
        ...
        const result = await createOrdersFromMaterialSelection(plan, expectedDelivery, organizationId || '', 'Komandni centar');
        if (result.ordersCreated > 0) { clearSelection(); onRefresh('orders', 'projects'); }
        return result;
    };
    ...
    const timelineActions = {
        onOpenWorkOrder: (id: string) => {
            ...
            document.querySelector('[data-panel="workorders"]')?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'nearest' });
        },
        onOpenTask: (id: string) => {
            const task = scope.tasks.find(t => t.Task_ID === id);
            document.querySelector('[data-panel="tasks"]')?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'nearest' });
            if (task) setSolo(null);
        },
        onOpenProject: (id: string) => onBoardChange({ ...board, Project_IDs: [id] }),
    };

    const soloVisible = (id: string) => !solo || solo === id;
    const otherPanels = solo ? Object.keys(SOLO_LABELS).filter(id => id !== solo) : [];
        ...
        <div className="kc-root">
            <header className="kc-top">
                ...
                <div className="kc-top-actions">
                    ...
                    {solo && <button type="button" className="kc-btn sm" onClick={() => setSolo(null)}>Vrati raspored</button>}
                    <button type="button" className="kc-toggle" aria-pressed={board.Show_Done} onClick={toggleShowDone}>
                        {board.Show_Done ? <Eye size={15} /> : <EyeOff size={15} />} Završeno
                    </button>
            ...
            <div className="kc-scroll">
                ...
                {onBoard.length === 0 ? (
                    ...
                    <>
                        ...
                        {solo && (
                            <div className="kc-solo-strip">
                                {otherPanels.map(id => (
                                    <button type="button" key={id} className="kc-solo-chip" onClick={() => setSolo(id)}>
                                        {SOLO_LABELS[id]}
                                    </button>
                                ))}
                            </div>
                            ...
                            <div className="kc-col kc-col-main">
                                {soloVisible('products') && (
                                    <ProductsPanel
                                        ...
                                        onOrderMaterials={() => setOrderModal(true)}
                                        solo={solo}
                                        onSolo={setSolo}
                                    />
                                )}
                                {soloVisible('workorders') && (
                                    <WorkOrdersPanel
                                        scope={scope}
                                        workers={workers}
                                        tasks={tasks}
                                        ...
                                        onNew={() => setWizardForEmpty(setWizardFor, onBoard)}
                                        canCreate={canCreate}
                                        solo={solo}
                                        onSolo={setSolo}
                                    />
            ...
            {orderModal && (
                <MaterialOrderSelectModal
                    ...
                    workOrderLabel={`Odabrani materijali · očekivana isporuka ${expectedDelivery}`}
...
/** „Novi nalog" bez predodabira — wizard sam nudi proizvode prvog projekta s table. */
function setWizardForEmpty(setter: (value: WizardInitialProducts | null) => void, projects: Project[]) {
    const project = projects[0];
    if (!project) return;
    setter({ projectId: project.Project_ID, projectName: project.Name || project.Client_Name || 'Projekat', products: [] });
}
